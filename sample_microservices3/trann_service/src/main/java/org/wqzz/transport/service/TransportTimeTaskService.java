...
//    ManifestRepository manifestRepository;
    ...
    //用于模拟上门取件，物流，以及货后付款
    @Async
    @Scheduled(initialDelay = 10000,fixedDelay = 30000) //定时任务在方法上的注解
    ...
    @Async
    @Scheduled(initialDelay = 15000,fixedDelay = 1000) //定时任务在方法上的注解
    ...
    public void finishManifest(){
        ...
        if(!list.isEmpty()){
            for (Transport transport:list){
                restTemplate.getForObject("http://MANI-SERVICE:8003/api/mani/manifest-finish-manifest/"+transport.getManifestId(),Integer.class);
            }
        }
    ...
    public void changeOneManifest(){
        ...
        if(!list.isEmpty()){
            for (Transport transport:list){
                restTemplate.getForObject("http://MANI-SERVICE:8003/api/mani/manifest-change-one-manifest/"+transport.getManifestId(),Integer.class);
